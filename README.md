...
- changing the frames per second in the functions (For this to work, you must call `pgzhelper_run.go()`, not `pgzrun.go()`)
...
You can only see "hi" because Screen is overloading the screen. With the init(), you would see both the "hi" and the "bye". For this example, Screen creating its own surface does affect the output.
